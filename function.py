...
def CalcolaSpeedy(cell):
    ...
    Speedy -= cell["dimension"] / 100
    return max(0,Speedy)

def ChangeDimension(cell, NewDimension):
    coloredCell = assents.cell.copy()
    coloredCell = pygame.transform.scale(
        coloredCell,
        (
    ...
    for i in cell["organels"]:   
        i["dimension"] = max(1, i["dimension"] + NewDimension - cell["dimension"])
    cell["dimension"] = NewDimension
    cell["rect"].width = NewDimension
    cell["rect"].height = NewDimension
...
def muteDNA(DNA):
    azione = random.choice(["remove", "remove", "duplicate", "duplicate", "changeColor", "changeColor", "changeColor", "changeColor","ChangeMR","ChangeMR","ChangeMR","ChangeSD","ChangeSD","ChangeSD"])
    ...
    if azione == "add":
        ...
        if len(DNA["cells"][Cell]["organnels"]) > 1:
    ...
    elif azione == "ChangeSD":
        DNA["cells"][Cell]["divisionDimension"] = max(0, DNA["cells"][Cell]["divisionDimension"] + random.randint(-5,5))
    return DNA
...
def canSplit(cell):
    ...
    if not mancanti:
        if cell["dimension"] >= cell["DNA"]["cells"][cell["nCell"]]["divisionDimension"]:
            return True
    return False
...
def split(cell):
    if canSplit(cell):
        ...
        ChangeDimension(cell, cell["dimension"] / 2)
...
def AggiungiRobeACaso(n):
    for i in range(random.randint(1,n)):
        type = assents.SostanzeSpawn[random.randint(0,len(assents.SostanzeSpawn)-1)]
...
def Crescita(cell):
    ...
    if a == 10 and cell["sostanze"].get("lipidi", 0) >= 1 and cell["sostanze"].get("Proteine", 0) >= 1:
        if "tilacoidi" in cell["DNA"]["cells"][cell["nCell"]]["organnels"] and random.randint(0,5) == 5:
            cell["sostanze"]["lipidi"] -= 0.3
            cell["sostanze"]["Proteine"] -= 0.3
            organnels.addOrganel(cell, "tilacoidiMat", random.randint(0,360))
        cell["sostanze"]["lipidi"] -= 0.1 * cell["DNA"]["cells"][cell["nCell"]]["MembraneResisten"]
        cell["sostanze"]["Proteine"] -= 0.1 * cell["DNA"]["cells"][cell["nCell"]]["MembraneResisten"]
        ChangeDimension(cell, cell["dimension"] + 1)

def decomposizione(cell):
    if random.randint(0,10) == 10:
        if "DecomState" in cell:
            cell["DecomState"] += 1
            if cell["DecomState"] == cell["dimension"] * 20:
...
def inglobate(cell):
    ...
    if "Enzimi" in cell["ParentCell"]["sostanze"]:
        ...
        if ToDamege == len(cell["organels"]):
            ChangeDimension(cell, max(0, cell["dimension"] - EnzimiPower))
