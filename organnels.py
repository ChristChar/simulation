...
def respirazioneAerobica(cell, organel):
    ...
    if a == 10 and "Proteine" in cell["sostanze"] and cell["sostanze"]["Proteine"] >= 1:
        numero_flagelli = sum(1 for org in cell["organels"] if org["type"] in ["mitocondro"])
        limite = 0
...
def Nucelo(cell):
    ...
    if a == 10 and "Proteine" in cell["sostanze"] and cell["sostanze"]["Proteine"] >= 1:
        numero_flagelli = sum(1 for org in cell["organels"] if org["type"] in ["ribosoma"])
        limite = 0
        for i in cell["DNA"]["cells"][cell["nCell"]]["organnels"]:
            if  i == "ribosoma":
                limite += 4
...
def vacuoloRP(cell, organel):
    a = random.randint(0,10)
    if a == 10 and "Proteine" in cell["sostanze"] and cell["sostanze"]["Proteine"] >= 1:
        numero_flagelli = sum(1 for org in cell["organels"] if org["type"] in ["vacuolo","vacuoloRPR"])
        limite = 0
        for i in cell["DNA"]["cells"][cell["nCell"]]["organnels"]:
            if  i == "vacuolo":
                limite += 2
...
def lisomaRP(cell, organel):
    numero_lisoma = sum(1 for org in cell["organels"] if org["type"] in ["lisoma"])
    limite = 0
    for i in cell["DNA"]["cells"][cell["nCell"]]["organnels"]:
        if  i == "lisoma":
            limite += 2
    if numero_lisoma < limite:
